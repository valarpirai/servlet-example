...
public class ScriptProcessor implements RequestProcessor {
  ...
  private static final int OPTIMIZATION_LEVEL =
      PropertiesUtil.getInt("script.optimizationLevel", -1);
  private static final long MAX_MEMORY_BYTES =
      PropertiesUtil.getLong("script.maxMemory", 10485760L); // 10 MB default
  private static final int INSTRUCTION_OBSERVATION_THRESHOLD =
      PropertiesUtil.getInt("script.instructionThreshold", 10000);
  ...
  private Object executeWithLimits(
    ...
    org.mozilla.javascript.ContextFactory factory =
        new org.mozilla.javascript.ContextFactory() {
          @Override
          protected void observeInstructionCount(Context cx, int instructionCount) {
            ...
            if (elapsed > timeoutMillis) {
              throw new Error(
                  "Script execution timeout exceeded: " + elapsed + "ms > " + timeoutMillis + "ms");
            }
