...
public class JsonDataProcessor implements RequestProcessor {
  ...
  public ProcessorResponse process(HttpServletRequest request)
    ...
    try {
      ...
      Map<String, Object> responseData = Map.of("received", parsedJson, "size", jsonBody.length());
      ...
      return ProcessorResponse.builder()
          .statusCode(400)
          .body(JsonUtil.errorResponse("Bad Request", "Malformed JSON: " + e.getMessage(), 400))
          .build();
    } catch (Exception e) {
      return ProcessorResponse.builder()
          .statusCode(500)
          .body(
              JsonUtil.errorResponse(
                  "Internal Server Error", "Error processing JSON: " + e.getMessage(), 500))
          .build();
    }
  }
